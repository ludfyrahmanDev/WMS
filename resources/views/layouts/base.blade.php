    ...
    <script>
        ...
        if (search) {
            search.addEventListener('keyup', function(e) {
                if (e.key === 'Enter') {
                    window.location.href = window.location.origin + window.location.pathname + '?search=' + e.target
                        .value;
                }
            });
        }
        ...
        if (perPage) {
            perPage.addEventListener('change', function(e) {
                window.location.href = window.location.origin + window.location.pathname + '?per_page=' + e.target
                    .value;
            });
        }
        ...
        function toCurrency(number) {
            ...
            // Mengformat angka menjadi string dengan format mata uang
            var formattedString = formatter.format(number);

            // Menghapus bagian desimal .00 dari string
