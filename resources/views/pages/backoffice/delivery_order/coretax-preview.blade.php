...
@section('subcontent')
    <div class="intro-y mt-8 flex flex-col gap-3">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold">Preview Export Coretax - Delivery Order</h2>
                <p class="mt-1 text-slate-500">Preview data pembelian sebelum export ke format Coretax</p>
            </div>
        ...
        <div class="intro-y col-span-12">
            ...
            <div class="mb-5 rounded-lg border border-slate-200 bg-white">
                <div class="border-b border-slate-200 bg-gradient-to-r from-primary/5 to-primary/10 p-5">
                    <h3 class="flex items-center text-lg font-bold text-primary">
                        ...
                        Informasi Faktur Pajak Masukan
                    </h3>
                </div>
                <div class="p-6">
                    <div class="grid grid-cols-12 gap-6">
                        <div class="col-span-12 lg:col-span-6">
                            <div class="space-y-4">
                                ...
                                <div class="flex items-start gap-3">
                                    <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10">
                                        <x-base.lucide class="h-5 w-5 text-primary" icon="FileCheck" />
                                    </div>
                                    <div>
                                        <div class="text-xs font-medium text-slate-500">Nomor Surat Jalan</div>
                                        <div class="mt-1 text-lg font-bold text-slate-900">{{ $previewData['surat_jalan'] }}</div>
                                    </div>
                                </div>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-start gap-3">
                                    <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-warning/10">
                                        <x-base.lucide class="h-5 w-5 text-warning" icon="Building2" />
                                    </div>
                                    <div class="flex-1">
                                        <div class="text-xs font-medium text-slate-500">Penjual (Supplier)</div>
                                        <div class="mt-1 text-lg font-bold text-slate-900">{{ $previewData['delivery_order']->supplier->name }}</div>
                                        <div class="mt-1 text-sm text-slate-600">{{ $previewData['delivery_order']->supplier->address ?? '-' }}</div>
                                    </div>
                                </div>

                                <div class="flex items-start gap-3">
                                    <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-success/10">
                                        <x-base.lucide class="h-5 w-5 text-success" icon="Building" />
                                    </div>
                                    <div class="flex-1">
                                        <div class="text-xs font-medium text-slate-500">Pembeli (Perusahaan)</div>
                                        <div class="mt-1 text-lg font-bold text-slate-900">{{ $previewData['delivery_order']->cv->name }}</div>
                                        <div class="mt-1 text-sm text-slate-600">{{ $previewData['delivery_order']->cv->address ?? '-' }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="rounded-lg border border-slate-200 bg-white p-6">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <a href="{{ route('delivery_order.coretax-export-csv', $previewData['delivery_order']->id) }}" 
                    ...
                    <a href="{{ route('delivery_order.coretax-export-xml', $previewData['delivery_order']->id) }}" 
                ...
                <div class="mt-6 rounded-lg bg-info/10 p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 text-sm text-slate-700">
                            <p class="font-medium text-info">Catatan Penting - Faktur Pajak Masukan:</p>
                            <ul class="mt-2 list-inside list-disc space-y-1">
                                ...
                                <li>Pastikan data NPWP supplier sudah benar sebelum export</li>
                                <li>Tarif PPN yang digunakan adalah 11% sesuai peraturan terbaru</li>
                                <li>Data ini merupakan Faktur Pajak Masukan untuk pencatatan pembelian dari supplier</li>
                                <li>Nomor Faktur dan Surat Jalan harus sesuai dengan dokumen fisik dari supplier</li>
                            </ul>
                        </div>
                    </div>
                </div>
